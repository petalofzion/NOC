...
### Path A: The "Safe" Pivot (ε-Stability)
...
### The Problem
Prove $\Delta^2 U \le c_U Δ(\text{Energy}) - κ_U Δ(\text{Loss})$ for HB on Quadratics.
...
### The Problem
Coupled update $\theta_{n+1} = $, $\phi_{n+1} = $ with $\gamma_n / β_n \to 0$.
...
3.  **Lemma E' (The General Metric):** "Interaction Sign Determines Empowerment Change."
    *   *Formalism:* Define **O-Information ($Ω$)** or Spectral Gap.
    *   *Theorem:* If $Ω < 0$ (Synergy), Empowerment Collapses (Original Lemma E). If $Ω > 0$ (Interference), Empowerment Increases.
...
### The Opportunity
*   **Generalized SLLN:** Prove $\frac{1}{n} ∑ M_i \to 0$ a.s. for general MDS with bounded variance.
...
### The Gem
`NOC/E/Boundary/LoewnerHelpers.lean` proves $\log   $ monotonicity.
...
### The Formalism
1.  **Lazy Regime (NTK):** High Redundancy ($Ω > 0$), Linear Dynamics, Additive Features. Lemma E is False here.
2.  **Rich Regime (Feature Learning):** High Synergy ($Ω < 0$), Non-linear, Simplex Geometry. Lemma E is True here.
3.  **The Meta-Utility Function:** $M(\pi) = \alpha Δ U + β Δ^2 U + γ Σ$.

### The Theorem
**"If $\gamma$ is sufficiently large, the Global Maximum of $M(\pi)$ lies in the Rich Regime."**
*   *Proof Sketch:* Show that in the Lazy regime, $Σ$ is bounded by additivity. In the Rich regime, $Σ$ scales with the combinatorics of the Simplex. Thus, for high $\gamma$, the Rich regime dominates.
...
*   **Hypothesis:** NOC acts as an **"Edge of Stability" (EoS)** controller. Maximizing $Δ^2 U$ (acceleration) pushes the system to the limit of stable curvature.
...
2.  **The "Rank Deficient" Theorem:**
    *   Define a system as "Synergistic" (Non-Holonomic) if the rank of the control distribution $\Delta = 	ext{span}\{f_1, , f_k\}$ is less than the rank of the Lie Algebra $	ext{Lie}(\Delta)$.
